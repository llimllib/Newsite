(() => {
  ...
  function map2(mapData, populationData) {
    const width = 975, height = 610, scale = log().domain(extent(Object.values(populationData))), colorScale = (d) => Greys_default(scale(d)), projection = albersUsa_default().scale(1300).translate([487.5, 305]);
  ...
  onready(async (event) => {
    map2(...await Promise.all([
      json_default(`https://cdn.jsdelivr.net/npm/us-atlas@3/states-10m.json`),
