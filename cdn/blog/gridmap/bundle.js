(() => {
  ...
  function map2(populationData) {
    ...
    const svg = select_default2("#map").append("svg").attr("width", width).attr("height", height).attr("viewBox", [0, 0, width, height]).attr("style", "width: 100%; height: auto; height: intrinsic;");
    const statesg = svg.append("g").selectAll("g").data(Object.values(states)).join("g");
    statesg.append("rect").attr("x", (d) => d.x * (squareSize + padding * 2)).attr("y", (d) => d.y * (squareSize + padding * 2)).attr("width", squareSize).attr("height", squareSize).attr("state", (d) => d.name).attr("fill", (d) => colorScale(populationData[d.name]));
  ...
  onready(async (event) => map2(await json_default(`https://cdn.billmill.org/static/blog/us_choro/population.json`)));
})();
