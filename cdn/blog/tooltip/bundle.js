(() => {
  ...
  function match(grid2, states2) {
    for (row = 0; row < grid2.length; row++) {
      for (col = 0; col < grid2[0].length; col++) {
        if (grid2[row][col] !== "  ") {
          states2[grid2[row][col]].y = row;
          states2[grid2[row][col]].x = col;
        }
      }
    }
  }
  ...
  function makeUSMap(target, populationData) {
    const width = 975, height = 610, padding = 2, scale = log().domain(extent(Object.values(populationData))).range([0.3, 1]), colorScale = (d) => Blues_default(scale(d)), cols = grid[0].length, rows = grid.length, squareSize = min([
      (width - cols * padding * 2) / cols,
      (height - rows * padding * 2) / rows
    ]);
    Object.values(states).forEach((s) => s.population = populationData[s.name]);
    const svg = select_default2(target).append("svg").attr("width", width).attr("height", height).attr("viewBox", [0, 0, width, height]).attr("style", "width: 100%; height: auto; height: intrinsic;");
    const statesg = svg.append("g").selectAll("g").data(Object.values(states)).join("g").attr("class", "state");
    statesg.append("rect").attr("x", (d) => d.x * (squareSize + padding * 2)).attr("y", (d) => d.y * (squareSize + padding * 2)).attr("width", squareSize).attr("height", squareSize).attr("state", (d) => d.name).attr("fill", (d) => colorScale(d.population));
    statesg.append("text").attr("x", (d) => d.x * (squareSize + padding * 2) + squareSize / 2).attr("y", (d) => d.y * (squareSize + padding * 2) + squareSize / 2).attr("fill", "white").style("text-anchor", "middle").attr("dominant-baseline", "central").text((d) => d.key);
    return svg;
  }
  pop_prom = json_default(`https://cdn.billmill.org/static/blog/us_choro/population.json`);
  function onready(f) {
  ...
  onready(async (event) => {
    ...
    let states2 = map1.selectAll("g.state");
    states2.append("title").text((d) => d.name);
    const map2 = makeUSMap("#map2", popData);
    const tooltip = map2.append("g").attr("class", "tooltip").style("pointer-events", "none");
    states2 = map2.selectAll("g.state").on("pointerenter pointermove", (evt) => {
      [mouseX, mouseY] = pointer_default(evt);
      const target = select_default2(evt.target).datum(), name = target.name, pop = target.population, msg = `${name}
population: ${format(",")(pop)}`, padding = 4;
      tooltip.style("display", null);
      const bg = tooltip.selectAll("rect").data([,]).join("rect").attr("fill", "#ffffff").attr("fill-opacity", "0.9");
      const txt = tooltip.selectAll("text").data([,]).join("text").attr("dominant-baseline", "central").call((text) => text.selectAll("tspan").data(msg.split(/\n/)).attr("fill", "black").style("text-anchor", "middle").join("tspan").attr("x", mouseX).attr("y", (_, i) => mouseY + 30 * i).text((d) => d));
      const bbox = txt.node().getBBox(), width = bbox.width + padding * 2, height = bbox.height + padding * 2;
      bg.attr("height", height).attr("width", width).attr("x", mouseX - width / 2).attr("y", mouseY - height / 4);
    }).on("pointerleave", (evt) => {
      tooltip.style("display", "none");
    }).on("touchstart", (evt) => evt.preventDefault());
    const map3 = makeUSMap("#map3", popData);
    const tooltipdiv = select_default2("#map3").append("div").attr("class", "tooltip").style("display", "none").style("background", "rgba(69,77,93,.9)").style("border-radius", ".2rem").style("color", "#fff").style("padding", ".6rem").style("position", "absolute").style("text-overflow", "ellipsis").style("white-space", "pre").style("line-height", "1em").style("z-index", "300");
    states2 = map3.selectAll("g.state").on("pointerenter pointermove", (evt) => {
      const target = select_default2(evt.target).datum();
      tooltipdiv.style("display", null).html(`${target.name}<br>population: ${format(",")(target.population)}`).style("top", evt.pageY - 10 + "px").style("left", evt.pageX + 10 + "px");
    }).on("pointerleave", (evt) => {
